...
public class CBIRView {
	...
	private JMenuBar createMenuBar() {
		...
		ActionListener testMenuListener = (ActionEvent e) -> {
			controller.triggerTests(e.getActionCommand());
		};
		...
		mI_test.addActionListener((ActionEvent e) -> {
			controller.triggerTests("All");
		});
		menuBar.add(mI_test);
		...
		final JSliderDecimal weightSlider = JSliderDecimal.createDoubleJSlider(0.0, 1.0, 0.5, 1);
		...
		weightSlider.addChangeListener((ChangeEvent e) -> {
			if (!weightSlider.getValueIsAdjusting())
				controller.getSettings().setWeight(weightSlider.getDecimalValue());
		});
		m_weightValue.add(weightSlider);
		settingsMenu.add(m_weightValue);

		// Menupunkt "Number of Regions"
		JMenu m_regionsValue = new JMenu("Regions");
		final JSliderDecimal regionSlider = JSliderDecimal.createDoubleJSlider(0, 10, 3, 0);
		regionSlider.setMajorTickSpacing(1);
		// satSlider.setMinorTickSpacing(0.1);
		regionSlider.setPaintTicks(true);
		regionSlider.setPaintLabels(true);
		regionSlider.addChangeListener((ChangeEvent e) -> {
			if (!regionSlider.getValueIsAdjusting())
				controller.getSettings().setRegions(regionSlider.getDecimalValue());
		});
		m_regionsValue.add(regionSlider);
		settingsMenu.add(m_regionsValue);
