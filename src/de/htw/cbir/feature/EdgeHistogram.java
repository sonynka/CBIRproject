...
//		RenderedImage ri = image.getDisplayImage();
//		SampleModel sampleModel = RasterFactory.createBandedSampleModel(DataBuffer.TYPE_BYTE,width,height,1); // One band.
//		TiledImage tiledImage = new TiledImage(0,0,width,height,0,0,sampleModel,null);
//		ROI roi = new ROI(tiledImage);
//		PlanarImage img = roi.getAsImage();
//		boolean boo = roi.contains(0, 0);
		...
		int numOfRegions = (int)getRegionsSetting();
		int regHeight = height/numOfRegions;
		int n = width * regHeight;
		
		float[] featureVector = new float[3 * 3 * numOfRegions];
	...
	private float getRegionsSetting() {
		return settings.getRegions();
	}
